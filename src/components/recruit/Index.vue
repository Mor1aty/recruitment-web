...
export default {
  ...
  data() {
    return {
      ...
      banners: [
        ...
        {id: 2, url: require("../../assets/images/banner2.jpg")},
  ...
  methods: {
    ...
    handleSalary(salary) {
      this.salaryRequire = salary.condition
      let minIndex = -1;
      let maxIndex = -1;
      for (let i = 0; i < this.query.wheres.length; i++) {
        if (this.query.wheres[i].key === 'min_salary') {
          minIndex = i
        }
        if (this.query.wheres[i].key === 'max_salary') {
          maxIndex = i
        }
      }
      if (minIndex >= 0) {
    ...
    handleJobType(jobType) {
      this.jobTypeStr = jobType.name
      let jobTypeIndex = -1;
      for (let i = 0; i < this.query.wheres.length; i++) {
        if (this.query.wheres[i].key === 'type') {
          jobTypeIndex = i
        }
      }
      if (jobTypeIndex >= 0) {
    ...
    chooseCity(city) {
      ...
      let cityIndex = -1;
      for (let i = 0; i < this.query.wheres.length; i++) {
        if (this.query.wheres[i].key === 'j.city') {
          cityIndex = i
        }
      }
      if (cityIndex >= 0) {
